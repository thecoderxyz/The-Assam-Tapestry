...
export default function StoryBox() {
  ...
  return (
    <div className="w-full mb-8 mx-4">
      <div 
        className={`w-full max-w-4xl mx-auto animate-fade-in bg-gradient-to-br ${theme.gradient} backdrop-blur-lg border-2 ${theme.border} rounded-xl p-6 shadow-2xl ${theme.glow}`}
      >
        {/* Chapter indicator */}
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-teal-400 text-sm font-semibold animate-pulse-slow">
              {chapter.title}
            </span>
          </div>
            ...
            {['chapter1', 'chapter2', 'chapter3'].map((ch, idx) => (
              <div
                ...
                className={`w-2 h-2 rounded-full transition-all duration-300 ${
                  ch === currentChapter ? 'bg-teal-400 w-6' : 'bg-gray-600'
                }`}
              />
            ))}
          </div>
        ...
        {/* Image Container */}
        <div className="mb-6 rounded-lg overflow-hidden border-2 border-gray-600/50 shadow-xl animate-slide-in">
          <img 
            src={stage.image} 
            alt="Scene" 
        ...
        {/* Story Text */}
        <div className="text-lg leading-relaxed mb-6 text-gray-200 bg-gray-900/30 p-4 rounded-lg animate-slide-in">
          {stage.text}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-slide-in">
          {stage.choices.map((choice: any, index: number) => (
            <button
              ...
              className="group relative w-full bg-gradient-to-br from-gray-800 to-gray-900 p-4 rounded-lg shadow-md text-left text-gray-200 border-2 border-gray-600 transition-all duration-300 hover:bg-gradient-to-br hover:from-teal-800 hover:to-gray-800 hover:scale-105 hover:shadow-xl hover:border-teal-400 overflow-hidden"
            >
              ...
              <span className="relative z-10 font-medium">{choice.text}</span>
            </button>
          ))}
        </div>
        ...
        <div className="mt-6 text-center">
          <button
            ...
            className="bg-gradient-to-r from-teal-600 to-teal-700 text-white px-8 py-3 rounded-lg shadow-lg hover:shadow-teal-500/50 hover:from-teal-500 hover:to-teal-600 transition-all duration-300 text-sm font-semibold hover:scale-105"
          >
            💾 Save Progress
          </button>
        </div>
